...
public:
    ...
    void SetDamageMultiplier(float multiplier) { damageMultiplier_ = std::max(0.0f, multiplier); }
    float GetDamageMultiplier() const { return damageMultiplier_; }
    void SetAutoGather(bool enabled) { autoGather_ = enabled; }
    bool IsAutoGathering() const { return autoGather_; }
    void SetAutoFreezeWhenIdle(bool enabled) { autoFreezeWhenIdle_ = enabled; }
    void SetAttackTarget(GameEntity* target) { attackTarget_ = target; }
    GameEntity* GetAttackTarget() const { return attackTarget_; }
    void ClearAttackTarget() { attackTarget_ = nullptr; }
...
protected:
    ...
    bool autoFreezeWhenIdle_ = false;
    GameEntity* attackTarget_ = nullptr;
};
