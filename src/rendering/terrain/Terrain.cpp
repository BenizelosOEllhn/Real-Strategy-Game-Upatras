...
auto carveRiver = [&](float startX, float dir)
{
    ...
    // Keep the wider mid-section, but taper near source to avoid collision artifacts.
    ...
    halfWidth = (halfWidth + midBoostA + midBoostB + tailBoost) * sourceTaper;
    ...
    float depth = 4.5f + 6.5f * (u * u);
    float carveAmount = depth * std::pow(u, 0.60f);
    y -= carveAmount;
    if (y < minRiverH)
        y = minRiverH;
};
...
void Terrain::SyncVerticesToGPU()
{
    if (!VAO || !VBO) {
        std::cerr << "[Terrain] ERROR: VAO or VBO not initialized\n";
        return;
    }
...
void Terrain::RecalculateNormalsInRadius(const glm::vec3& center, float radius)
{
    if (vertices.empty()) {
        return;
    }

    int minX = std::max(0, static_cast<int>(std::floor(center.x + width * 0.5f - radius)) - 1);
    int maxX = std::min(width, static_cast<int>(std::ceil(center.x + width * 0.5f + radius)) + 1);
    int minZ = std::max(0, static_cast<int>(std::floor(center.z + depth * 0.5f - radius)) - 1);
    int maxZ = std::min(depth, static_cast<int>(std::ceil(center.z + depth * 0.5f + radius)) + 1);
    float radiusSq = radius * radius;
    int stride = width + 1;

    auto vertexHeight = [&](int x, int z) -> float {
        return vertices[static_cast<size_t>(z) * static_cast<size_t>(stride) + static_cast<size_t>(x)].Position.y;
    };

    for (int z = minZ; z <= maxZ; ++z) {
        for (int x = minX; x <= maxX; ++x) {
            size_t idx = static_cast<size_t>(z) * static_cast<size_t>(stride) + static_cast<size_t>(x);
            float dxCenter = vertices[idx].Position.x - center.x;
            float dzCenter = vertices[idx].Position.z - center.z;
            if (dxCenter * dxCenter + dzCenter * dzCenter > radiusSq) {
                continue;
            }

            int xL = std::max(0, x - 1);
            int xR = std::min(width, x + 1);
            int zD = std::max(0, z - 1);
            int zU = std::min(depth, z + 1);

            glm::vec3 dxVec(
                static_cast<float>(xR - xL),
                vertexHeight(xR, z) - vertexHeight(xL, z),
                0.0f
            );
            glm::vec3 dzVec(
                0.0f,
                vertexHeight(x, zU) - vertexHeight(x, zD),
                static_cast<float>(zU - zD)
            );

            glm::vec3 normal = glm::cross(dzVec, dxVec);
