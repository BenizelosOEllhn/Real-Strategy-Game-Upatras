...
bool Scene::findPath(const glm::vec3& start, const glm::vec3& goal, std::vector<glm::vec3>& outPath) const
{
    ...
    while (!open.empty())
    {
        ...
        for (const auto& offset : offsets)
        {
            ...
            float stepCost = (offset[0] == 0 || offset[1] == 0) ? navCellSize_ : navCellSize_ * 1.4142f;
            float terrainCost = (navWalkable_[neighborIdx] == static_cast<uint8_t>(NavNodeState::HighCost)) ? 2.0f : 1.0f;
            float tentative = gScore[current.idx] + stepCost * terrainCost;
...
glm::vec3 Scene::navToWorld(int col, int row) const
{
    ...
    float y = terrainDeformer_ ? terrainDeformer_->GetHeightAtPoint(x, z) : Terrain::getHeight(x, z);
