...
namespace {
    ...
    constexpr float kLakeRadius = 55.0f; // generous radius to keep shoreline clear
...
void Scene::Draw(Shader& terrainShader,
    ...
    if (hasRockMeshes) {
        ...
        for (const auto& transform : rockTransforms) {
            objectShader.SetMat4("model", transform);
...
void Scene::generateTrees() {
    ...
    while (treeTransforms.size() < kTreeCount && attempts < maxAttempts) {
        ...
        // Avoid placing non-mountain trees too far into mountains
        ...
        if (height < -2.0f) continue; // skip underwater spots
        if (mountainBand && height < 8.0f) continue; // don't plant deep in riverbeds
