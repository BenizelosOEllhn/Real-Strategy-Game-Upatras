...
bool Model::loadWithAssimp(const char* path)
{
    ...
    std::cout << "[Model] Loading " << sourcePath_ << " with " << scene->mNumMeshes << " meshes\n";
    ...
    for (unsigned int i = 0; i < scene->mNumMaterials; ++i) {
        ...
        if (mat->Get(AI_MATKEY_NAME, matName) == AI_SUCCESS) {
            std::cout << "[Model]   Material " << i << ": " << matName.C_Str() 
    ...
    for (unsigned int i = 0; i < scene->mNumMaterials; ++i)
    {
        ...
        if (material->GetTexture(aiTextureType_DIFFUSE, 0, &texPath) == AI_SUCCESS)
        {
            ...
            if (texID != 0)
            {
                ...
                std::cout << "[Model]   Material " << i << " texture assigned (ID=" << texID << ")\n";
            }
            else
            {
                std::cout << "[Model]   Material " << i << " has no valid texture\n";
            }
        }
    }
    std::cout << "[Model] Texture loading complete. hasAnyTextures=" << hasAnyTextures_ << "\n";
}
...
unsigned int Model::loadTextureForMaterial(const std::string& relPath)
{
    ...
    std::cout << "[Model] Loading texture: " << fullPath << "\n";
    ...
    std::cout << "[Model] Texture loaded successfully (ID=" << texID << ")\n";
    return texID;
}
...
void Model::SetOverrideTexture(const std::string& path, bool flipY)
{
    if (path.empty())
        return;

    std::unique_ptr<Texture> tex = std::make_unique<Texture>(path.c_str(), flipY);
    unsigned int texID = tex->ID;
    ownedTextures_.push_back(std::move(tex));
